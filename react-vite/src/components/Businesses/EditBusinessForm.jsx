...
import { fetchAllBusinesses, updateBusiness } from '../../redux/businesses';
...
const EditBusinessForm = () => {
  ...
  const business = useSelector(state => state.businesses.allBusinesses[businessId]);
  ...
  // Fetch all businesses if not loaded
  useEffect(() => {
    if (!business) {
  ...
  // Populate form when business data is available
  useEffect(() => {
    if (business) {
      ...
      setFormData({
        ...
        price_range: business.price_range || 1,
        hours_monday: business.hours_monday || '',
        hours_tuesday: business.hours_tuesday || '',
        hours_wednesday: business.hours_wednesday || '',
        hours_thursday: business.hours_thursday || '',
        hours_friday: business.hours_friday || '',
        hours_saturday: business.hours_saturday || '',
        hours_sunday: business.hours_sunday || ''
      });
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await dispatch(updateBusiness(businessId, formData));
      navigate(`/businesses/${businessId}`);
    } catch (error) {
      setErrors({ submit: error.message || 'Failed to update business' });
    } finally {
      setIsSubmitting(false);
    }
  };

  // Show loading if business not loaded yet
  if (!business) {
    return (
